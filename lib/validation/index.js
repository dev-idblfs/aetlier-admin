...
export const mobilePromotionSchema = z.object({
    ...
    cta_type: z.enum(['route', 'service', 'package', 'external']).default('route'),
...
export const whatsAppIntegrationSchema = z
    ...
    .superRefine((data, ctx) => {
        ...
        if (data.whatsapp_enabled && !wabaId) {
            ctx.addIssue({
                code: 'custom',
                message: 'Business account ID (WABA) is required when WhatsApp is enabled',
                path: ['whatsapp_business_account_id'],
            });
        }
    });
...
export const organizationSelfSignupSchema = z.object({
    slug: z
        .string()
...
export const organizationOnboardSchema = z.object({
    slug: z
        ...
        .min(2, 'Slug is required')
        .regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/, 'Use lowercase letters, numbers, and hyphens'),
    name: requiredString('Legal name'),
    display_name: z.string().optional().or(z.literal('')),
    owner_email: emailSchema,
    owner_name: z.string().optional().or(z.literal('')),
    owner_password: z.string().min(8).optional().or(z.literal('')),
    phone: z.string().optional().or(z.literal('')),
    support_email: z.union([emailSchema, z.literal('')]).optional(),
    ...
    address_country: z.string().default('IN'),
    ...
    business_email: z.union([emailSchema, z.literal('')]).optional(),
    business_phone: z.string().optional().or(z.literal('')),
    business_address: z.string().optional().or(z.literal('')),
    business_gstin: z.string().optional().or(z.literal('')),
    business_pan: z.string().optional().or(z.literal('')),
    timezone: z.string().default('Asia/Kolkata'),
    locale: z.string().default('en-IN'),
    invoice_prefix: z.string().default('INV'),
    expense_prefix: z.string().default('EXP'),
    default_tax_rate: z.coerce.number().min(0).default(18),
    tax_type: z.string().default('GST'),
    enable_cgst_sgst: z.boolean().default(true),
    currency_code: z.string().default('INR'),
    currency_symbol: z.string().default('₹'),
    default_due_days: z.coerce.number().int().min(0).default(7),
    default_terms: z.string().optional().or(z.literal('')),
    default_notes: z.string().optional().or(z.literal('')),
    feature_whatsapp: z.boolean().default(false),
    feature_livekit: z.boolean().default(false),
    feature_facescan: z.boolean().default(false),
    feature_online_consultation: z.boolean().default(false),
    feature_referrals: z.boolean().default(false),
    feature_public_catalog: z.boolean().default(false),
    feature_public_booking: z.boolean().default(false),
    feature_booking_webhooks: z.boolean().default(false),
});
...
export const organizationSetupSchema = z.object({
    ...
    business_phone: z.string().optional().or(z.literal('')),
    business_address: requiredString('Business address'),
    business_gstin: z.string().optional().or(z.literal('')),
    business_pan: z.string().optional().or(z.literal('')),
    timezone: z.string().default('Asia/Kolkata'),
    locale: z.string().default('en-IN'),
    ...
    default_tax_rate: z.coerce.number().min(0).default(18),
    enable_cgst_sgst: z.boolean().default(true),
    invite_email: z.union([emailSchema, z.literal('')]).optional(),
});
