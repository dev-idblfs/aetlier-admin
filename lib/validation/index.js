...
export const doctorSchema = z.object({
  ...
  registration_council: z.string().max(200).optional().or(z.literal("")),
  rx_practice_address: z.string().max(2000).optional().or(z.literal("")),
});
...
export const mobilePromotionSchema = z.object({
  ...
  cta_type: z
    .enum(["route", "service", "package", "external"])
    .default("route"),
  cta_value: z.string().optional().or(z.literal("")),
  placement: z.enum(["banner", "offer"]).default("banner"),
...
export const serviceSchema = z
  ...
  .refine((data) => (data.selling_price ?? 0) > 0 || (data.price ?? 0) > 0, {
...
export const appointmentSchema = z.object({
  ...
  service_id: requiredString("Service"),
  doctor_id: requiredString("Doctor"),
  consultation_mode: z.enum(["in_person", "online"]).default("in_person"),
  preferred_date: requiredString("Date"),
  preferred_time: requiredString("Time"),
  special_notes: z.string().optional(),
  patient_user_id: z.string().optional().or(z.literal("")),
  patient_gender: z.string().optional().or(z.literal("")),
  patient_date_of_birth: z.string().optional().or(z.literal("")),
  patient_city: z.string().optional().or(z.literal("")),
  patient_address: z.string().optional().or(z.literal("")),
  media_mode: z.enum(["video", "audio"]).optional(),
});
...
export const customerSchema = z.object({
  ...
  billing_attention: z.string().optional().or(z.literal("")),
  ...
  billing_state_id: z.string().optional().or(z.literal("")),
  billing_city_id: z.string().optional().or(z.literal("")),
  billing_pincode: z.string().optional().or(z.literal("")),
  billing_country: z.string().optional().or(z.literal("")),
  shipping_attention: z.string().optional().or(z.literal("")),
  ...
  shipping_state_id: z.string().optional().or(z.literal("")),
  shipping_city_id: z.string().optional().or(z.literal("")),
...
export const settingsSchema = z.object({
  ...
  documentTagline: z.string().max(255).optional(),
...
export const invoiceSchema = z.object({
  ...
  doctor_user_id: z.preprocess(nullToOptionalString, z.string().optional()),
  ...
  customer_address: z.preprocess((val) => {
    if (val == null || val === "") return "";
    if (typeof val === "object") return JSON.stringify(val);
    return String(val);
  }, z.string().optional()),
  customer_date_of_birth: z.preprocess(
    nullToEmptyString,
    z.string().optional(),
  ),
  customer_gender: z.preprocess(nullToEmptyString, z.string().optional()),
  customer_city: z.preprocess(nullToEmptyString, z.string().optional()),
...
export const leadUpdateSchema = z.object({
  ...
  interest: z
    .string()
    .max(255, "Interest must be 255 characters or less")
    .optional()
    .or(z.literal("")),
...
export const whatsAppIntegrationSchema = z
  ...
  .superRefine((data, ctx) => {
    const phoneId = data.whatsapp_phone_number_id?.trim() || "";
    const wabaId = data.whatsapp_business_account_id?.trim() || "";

    if (wabaId && (wabaId.includes("@") || !/^\d+$/.test(wabaId))) {
      ctx.addIssue({
        code: "custom",
        message: "WABA ID must be numeric (from Meta), not an email",
    ...
    if (phoneId && !/^\d+$/.test(phoneId)) {
      ctx.addIssue({
        code: "custom",
        message: "Phone number ID must be numeric (from Meta API Setup)",
        path: ["whatsapp_phone_number_id"],
      });
    }

    if (data.whatsapp_enabled && !phoneId) {
      ctx.addIssue({
        code: "custom",
    ...
    if (data.whatsapp_enabled && !wabaId) {
      ctx.addIssue({
        code: "custom",
        message:
          "Business account ID (WABA) is required when WhatsApp is enabled",
        path: ["whatsapp_business_account_id"],
      });
    }
  });
