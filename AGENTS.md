...
- Permission strings use the backend dot-notation in the `PERMISSIONS` object (e.g. `appointment.read.any`, `mobile.promotion.read.any`). **Action may contain dots** — keep strings aligned with backend seed / `permission_strings` parse rules. Add new permissions there, keeping them aligned with the backend.
